...
export type DayNightProps = Omit<React.LabelHTMLAttributes<HTMLLabelElement>, 'onChange'> & {
  checked?: boolean;
  onChange?: (checked: boolean) => void;
  className?: string;
};

const DayNight: React.FC<DayNightProps> = ({ checked = false, onChange, className, ...rest }) => {
  return (
    <StyledWrapper className={className}>
      <label className="bb8-toggle" {...rest}>
        <input
          className="bb8-toggle__checkbox"
          type="checkbox"
          checked={checked}
          onChange={(e) => onChange?.(e.target.checked)}
          aria-label={checked ? 'Switch to day theme' : 'Switch to night theme'}
        />
