...
const Button = ({ href, children = 'Plant based', className, ...rest }: { href?: string; children?: React.ReactNode; className?: string } & React.HTMLAttributes<HTMLElement>) => {
  ...
  return (
    <StyledWrapper>
      {href ? (
        <Link href={href} className={`plant-btn ${className ?? ''}`} {...rest}>
          {Content}
        </Link>
      ) : (
        <button type="button" className={`plant-btn ${className ?? ''}`} {...rest}>
          {Content}
        </button>
      )}
    </StyledWrapper>
