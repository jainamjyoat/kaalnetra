...
void main() {
    ...
    // Disable velocity-based vertex stretching to avoid image deformation
    // Keeping discs perfectly round regardless of rotation
...
class InfiniteGridMenu {
  ...
  private initTexture(): void {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        if (iw > ih) {
          // Landscape: crop equally from left/right to make square
          sx = Math.max(0, Math.floor((iw - ih) / 2));
          sy = 0;
          sw = ih;
          sh = ih;
        } else if (ih > iw) {
          // Portrait: crop equally from top/bottom to make square
          sx = 0;
          sy = Math.max(0, Math.floor((ih - iw) / 2));
          sw = iw;
          sh = iw;
        }
  ...
  public pickItemAt(clientX: number, clientY: number): number {
    const canvas = this.gl?.canvas as HTMLCanvasElement;
    if (!canvas || !this.gl) return 0;
    ...
    for (let i = 0; i < this.instancePositions.length; ++i) {
      ...
      const worldPos = vec3.scale(vec3.create(), dir, -this.SPHERE_RADIUS);
      const p4 = vec4.fromValues(worldPos[0], worldPos[1], worldPos[2], 1);
      const clip = vec4.transformMat4(vec4.create(), p4, mvp);
    ...
    // Manual-only selection: do not set snap target or auto-callback
...
const Modal: FC<{ open: boolean; onClose: () => void; item?: MenuItem | null }> = ({ open, onClose, item }) => (
  <div className={`fixed inset-0 z-[60] transition ${open ? 'pointer-events-auto' : 'pointer-events-none'}`} aria-hidden={!open}>
    ...
    <div className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[90vw] max-w-xl rounded-2xl bg-neutral-900 text-white shadow-2xl border border-white/10 p-6 transition-all duration-300 ${open ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
        ...
        <div className="flex-1">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
            {(item?.features || []).map((f, i) => (
              <li
                key={i}
                className={`px-3 py-2 rounded-lg bg-white/5 border border-white/10 transition-all duration-300 ${open ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}
      ...
      <div className="mt-6 flex justify-end">
        <button onClick={onClose} className="px-4 py-2 rounded-lg bg-white text-black font-semibold hover:bg-white/90">Close</button>
      </div>
    </div>
  </div>
);
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (_index: number) => {
      // Disable any automatic active-item updates from the engine
      return;
    };
    ...
    if (canvas) {
      ...
      const handleCanvasClick = (e: MouseEvent) => {
        ...
        const idx = sketch.pickItemAt(e.clientX, e.clientY);
        lockActiveRef.current = true; // lock selection to clicked item
      ...
      const cleanup = () => canvas.removeEventListener('click', handleCanvasClick);
      (cleanup as any)._isCleanup = true;
      (window as any).__infiniteMenuCanvasCleanup = cleanup;
    }
    ...
    return () => {
      ...
      const cleanup = (window as any).__infiniteMenuCanvasCleanup;
      if (cleanup && typeof cleanup === 'function' && cleanup._isCleanup) cleanup();
  ...
  const handleCloseModal = () => {
    setOpenModal(false);
    lockActiveRef.current = false; // unlock after modal closes
  };
  ...
  return (
    <div className="relative w-full h-full">
      <canvas
        ...
        ref={canvasRef}
        className={`cursor-grab w-full h-full overflow-hidden relative outline-none active:cursor-grabbing ${openModal ? 'pointer-events-none' : ''}`}
      />
      ...
      {activeItem && (
        ...
        `}
          ...
          ${
            isMoving
              ? 'opacity-0 pointer-events-none duration-[100ms] scale-90'
              : 'opacity-100 pointer-events-auto duration-[500ms] scale-100'
          }
        `}
          >
            {/* <button className="faq-button" onClick={handleOpenModal} aria-label="Open flower details">
      ...
      <Modal open={openModal} onClose={handleCloseModal} item={activeItem} />
      {/* <style jsx>{`
        .faq-button {
          width: 50px;
          height: 50px;
        ...
        .faq-button:hover { animation: jello-vertical 1s both; }
        .faq-button svg { height: 1.5em; fill: white; }
